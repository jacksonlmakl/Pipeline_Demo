...
# Render Jinja content (used for attributes and inner content)
def render_jinja(content, variables):
...
def xml(xml_string, variables):
    ...
    for match in tag_pattern.finditer(xml_string):
        ...
        # Parse and render attributes with Jinja
        for attr_match in attr_pattern.finditer(attributes):
            key = attr_match.group('key')
            value = attr_match.group('value')
            # Render attribute values with Jinja2
            rendered_value = render_jinja(value, variables)
            tag_dict[key] = rendered_value

        # Render inner content with Jinja2
        rendered_content = render_jinja(content, variables)
